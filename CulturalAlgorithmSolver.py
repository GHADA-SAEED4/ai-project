# CulturalAlgorithmSolver.py - Enhanced Version with GA & CA
# Includes: plateau detection, restart mechanism, local search, timeout, and adaptive parameters
# Compatible with existing Solver interface
...
class Individual:
    ...
    def calculate_fitness(self):
        ...
        if self.conflicts == 0:
            ...
            # Stronger exponential penalty for better convergence
...
class BeliefSpace:
    ...
    def __init__(self, size: int):
        ...
        # Track stagnation for plateau detection
        self.stagnation_counter = 0
        self.last_best_conflicts = float('inf')

    def update(self, accepted_individuals: List[Individual]):
        """Update belief space with weighted emphasis on better solutions"""
        for ind in accepted_individuals:
            ...
            # Weight by squared fitness to emphasize better solutions
    ...
    def influence(self, individual: Individual, fixed_mask: List[List[bool]], 
        ...
        # Reduce influence when stagnant to allow exploration
            ...
            for j in range(self.size):
                if not fixed_mask[i][j]:
                    ...
                    if prefs and random.random() < actual_rate:
                        ...
                        if is_stagnant:
                            # When stuck: try random valid numbers or less popular choices
...
class ParameterOptimizer:
    ...
    def get_optimal_params(size: int, difficulty: float = 0.5) -> Dict:
        """Get optimal parameters for given board size"""
        ...
        return {
            ...
            'max_time': 120  # Maximum time in seconds
        }


class CulturalAlgorithmSolver(Solver):
    """Enhanced Cultural Algorithm with belief space, plateau handling, and restart"""
    
    def __init__(self, board, pop_size=None, generations=None):
        """Initialize solver - compatible with existing interface"""
        ...
        self.restart_threshold = params['restart_threshold']
        self.max_time = params['max_time']
        
        # Track which cells are fixed (given)
        ...
        # Tracking for plateau detection
    ...
    def _restart_population(self, keep_best: int = 5):
        """Restart population with diversity injection while keeping best solutions"""
        best_individuals = self.population[:keep_best]
        
        # Apply intensive local search to best individuals
        for ind in best_individuals:
            if ind.conflicts <= 5:
                self._local_search(ind, max_iterations=30)
        
        # Reduce confidence in old belief patterns
        for i in range(self.size):
            for j in range(self.size):
        ...
        # Keep best and create new diverse population
    ...
    def _selection(self) -> Individual:
        """Tournament selection with adaptive randomness"""
    ...
    def _local_search(self, individual: Individual, max_iterations: int = 50):
        """Intensive local search for near-solution individuals"""
        board = individual.board
        improved = True
        iterations = 0
        ...
        while improved and iterations < max_iterations and individual.conflicts > 0:
            ...
            # Sort by conflict count (highest first)
            conflict_cells.sort(reverse=True)
            
            # Try to fix top conflict cells
    ...
    def _mutate(self, individual: Individual):
        """Smart mutation with local search for near-solutions"""
        # Apply intensive local search if close to solution
        if individual.conflicts <= 5 and individual.conflicts > 0:
            self._local_search(individual, max_iterations=50)
            if individual.conflicts == 0:
                return
    ...
    def _count_cell_conflicts(self, board, row: int, col: int) -> int:
        """Count conflicts for a specific cell"""
        conflicts = 0
    ...
    def solve(self):
        """Main solving function - compatible with existing interface"""
        self.metrics.start()
        self.is_running = True
        ...
        for gen in range(self.max_generations):
            ...
            # Apply local search if close to solution
            if best.conflicts <= 10 and best.conflicts > 0:
                self._local_search(best, max_iterations=50)
            ...
            # Restart if stuck in plateau
            if self.stagnation_counter >= self.restart_threshold:
                self._restart_population(keep_best=max(3, self.pop_size // 20))
        ...
        if best.conflicts <= 15:
            self._local_search(best, max_iterations=100)
            if best.conflicts == 0:
                self.metrics.stop(True)
                return True, best.board
